...
export const InstructionGenerator: React.FC<InstructionGeneratorProps> = ({ printer, onClose }) => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            link.download = `uniprintpro-instrukcija-${printer.name.replace(/\s+/g, '-').toLowerCase()}.png`;
            link.href = dataUrl;
            link.click();
        } catch (err) {
    ...
    return (
        <div className="fixed inset-0 bg-slate-900/90 z-[200] overflow-y-auto flex flex-col items-center py-10 px-4 backdrop-blur-sm">
            ...
            <div className="w-full max-w-[210mm] flex justify-between items-center mb-6">
                ...
                <div className="flex gap-4 border border-slate-700 bg-slate-800/80 p-2 rounded-2xl shadow-xl">
                    <button
            ...
            <div className="shadow-2xl ring-1 ring-slate-900/5 bg-white relative">
                ...
                <div
                    ref={printRef}
                    className="bg-white text-slate-900 flex flex-col justify-between relative"

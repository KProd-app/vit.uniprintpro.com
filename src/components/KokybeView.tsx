...
import { PrinterLog, PrinterData, PrinterStatus, VITData } from '../types';
...
import { CheckCircle2, XCircle, ChevronLeft, ChevronRight, Camera, Pen, FileText, AlertTriangle, RefreshCw, Activity } from 'lucide-react';
...
interface PhotoLightboxProps { url: string; label: string; onClose: () => void; }
const PhotoLightbox: React.FC<PhotoLightboxProps> = ({ url, label, onClose }) => (
  <div className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-4" onClick={onClose}>
    ...
    <img src={url} alt={label} className="max-w-full max-h-[85vh] rounded-2xl shadow-2xl object-contain" onClick={e => e.stopPropagation()} />
...
interface PhotoGridProps { photos: { url: string; label: string }[]; printerName: string; }
const PhotoGrid: React.FC<PhotoGridProps> = ({ photos, printerName }) => {
  ...
  return (
    <div className="mt-3">
      ...
      <div className="flex flex-wrap gap-2">
        {photos.map(p => (
          <button key={p.url} onClick={() => setLightbox(p)} className="relative group active:scale-95 transition-transform">
            <img src={p.url} alt={p.label} className="w-28 h-20 object-cover rounded-xl border-2 border-slate-200 group-hover:border-mimaki-blue transition-all shadow-sm" />
      ...
      {lightbox && <PhotoLightbox url={lightbox.url} label={lightbox.label} onClose={() => setLightbox(null)} />}
    </div>
  );
};

// ── Shared VIT checklist display ──────────────────────────────────────────────

interface VITDisplayProps { vit: VITData; }
const VITDisplay: React.FC<VITDisplayProps> = ({ vit }) => {
  const checklist = vit?.checklist || {};
  ...
  const allGood = pct === 100 && vit?.confirmed && !!vit?.signature;

  return (
    <div className="mt-3">
      <div className="flex items-center gap-2 mb-2">
        <div className="flex-1 bg-slate-100 rounded-full h-2">
          <div className={`h-2 rounded-full transition-all ${pct === 100 ? 'bg-emerald-500' : pct >= 70 ? 'bg-yellow-400' : 'bg-red-400'}`} style={{ width: `${pct}%` }} />
        </div>
        ...
        {items.sort(([, a], [, b]) => (a === b ? 0 : a ? 1 : -1)).map(([raw, done]) => {
          ...
          return (
            <div key={raw} className={`flex items-start gap-2 text-sm px-2 py-1 rounded-lg ${done ? 'text-slate-500' : 'bg-red-50 text-red-700 font-semibold'}`}>
              {done ? <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" /> : <XCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />}
              <span>{text}</span>
            </div>
          );
        })}
      ...
      <div className="flex gap-2 mt-3 flex-wrap">
        <div className={`flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-lg ${vit?.confirmed ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
          {vit?.confirmed ? <CheckCircle2 className="w-3.5 h-3.5" /> : <XCircle className="w-3.5 h-3.5" />}
          Patvirtinta
        </div>
        <div className={`flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-lg ${vit?.signature ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
          <Pen className="w-3.5 h-3.5" />
          {vit?.signature ? 'Parašas ✓' : 'Parašo nėra'}
        </div>
        {!allGood && (
          <div className="flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-lg bg-orange-50 text-orange-700">
...
// ── Active printer card (live, shift not finished) ────────────────────────────

interface ActiveCardProps { printer: PrinterData; }
const ActiveCard: React.FC<ActiveCardProps> = ({ printer }) => {
  const [expanded, setExpanded] = useState(false);

  const photos: { url: string; label: string }[] = [];
  if (printer.nozzleFile?.url) photos.push({ url: printer.nozzleFile.url, label: `${printer.name} — Nozzle check` });
  if (printer.mimakiNozzleFiles) {
    Object.entries(printer.mimakiNozzleFiles).forEach(([unit, f]) => {
      if (f?.url) photos.push({ url: f.url, label: `${printer.name} — Blokas ${unit}` });
    });
  }

  const checklist = printer.vit?.checklist || {};
  const items = Object.entries(checklist);
  const checkedCount = items.filter(([, v]) => v).length;
  const vitOk = items.length > 0 && checkedCount === items.length && printer.vit?.confirmed && !!printer.vit?.signature;
  const hasNozzle = photos.length > 0;

  return (
    <div className="bg-white rounded-2xl shadow-sm border-2 border-blue-200">
      <button className="w-full text-left p-4 flex items-start gap-3" onClick={() => setExpanded(e => !e)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            <span className="font-black text-slate-800 text-base truncate">{printer.name}</span>
            <span className="text-xs font-bold px-2 py-0.5 rounded-full shrink-0 bg-blue-100 text-blue-700 flex items-center gap-1">
              <Activity className="w-3 h-3" /> Vyksta
            </span>
          </div>
          <div className="flex items-center gap-3 mt-1 text-sm text-slate-500">
            <span className="font-semibold text-slate-700 truncate">{printer.operatorName || '–'}</span>
            {printer.workStartedAt && <span className="shrink-0">nuo {formatTime(printer.workStartedAt)}</span>}
          </div>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-100 pt-3">
          ...
          {items.length > 0
            ? <VITDisplay vit={printer.vit} />
            : <p className="text-sm text-slate-400 italic">VIT dar nepradėta</p>
          }
          <PhotoGrid photos={photos} printerName={printer.name} />
        </div>
      )}
    </div>
  );
...
interface LogCardProps { log: PrinterLog; }
const LogCard: React.FC<LogCardProps> = ({ log }) => {
  const [expanded, setExpanded] = useState(false);

  const photos: { url: string; label: string }[] = [];
  if (log.nozzleData?.url) photos.push({ url: log.nozzleData.url, label: `${log.printerName} — Nozzle check` });
  ...
  const items = Object.entries(log.vitData?.checklist || {});
  const checkedCount = items.filter(([, v]) => v).length;
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border-2 transition-all ${vitOk ? 'border-emerald-200' : 'border-orange-200'}`}>
      ...
      {expanded && (
        <div className="px-4 pb-4 border-t border-slate-100 pt-3">
          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">VIT kontrolinis sąrašas</p>
          <VITDisplay vit={log.vitData} />
          <PhotoGrid photos={photos} printerName={log.printerName} />
        </div>
      )}
    </div>
  );
...
export const KokybeView: React.FC = () => {
  const { getShiftLogs, printers } = usePrinters();
  ...
  // Active printers: not idle, shown only when viewing today
  const activePrinters = isToday
    ? printers
        .filter(p => p.status !== PrinterStatus.NOT_STARTED)
  ...
  const hasAnyContent = activePrinters.length > 0 || logs.length > 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 shadow-sm">
        <div className="max-w-lg mx-auto px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setDate(d => addDays(d, -1))} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 transition-all">
            ...
            <button onClick={() => setDate(d => addDays(d, 1))} disabled={isToday} className="p-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-600 transition-all disabled:opacity-30 disabled:cursor-not-allowed">
            ...
            <div className="flex gap-2 mt-3 flex-wrap">
              {activePrinters.length > 0 && (
                <span className="text-xs font-bold bg-blue-100 text-blue-700 px-2.5 py-1 rounded-full flex items-center gap-1">
                  <Activity className="w-3 h-3" /> {activePrinters.length} aktyvūs
                </span>
              )}
              <span className="text-xs font-bold bg-slate-100 text-slate-600 px-2.5 py-1 rounded-full">
                {logs.length} užbaigtos pamainos
              </span>
              {logs.length > 0 && (
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${vitOkCount === logs.length ? 'bg-emerald-100 text-emerald-700' : 'bg-orange-100 text-orange-700'}`}>
                  {vitOkCount}/{logs.length} VIT atlikta
                </span>
              )}
            </div>
          )}
      ...
      <div className="max-w-lg mx-auto px-4 py-4 pb-12">
        ...
        {/* Active printers — today only */}
        {!loading && !error && activePrinters.length > 0 && (
          <section className="mb-6">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-base">⚡</span>
              <h2 className="text-sm font-black text-blue-600 uppercase tracking-widest">Šiuo metu vyksta</h2>
            </div>
            <div className="space-y-3">
              {activePrinters.map(p => <ActiveCard key={p.id} printer={p} />)}
            </div>
          </section>
        )}
